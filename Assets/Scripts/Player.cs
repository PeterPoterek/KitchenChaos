...
public class Player : MonoBehaviour, IKitchenObjectParent
{
 ...
 private void HandleMovement()
 {
  ...
  if (!canMove)
  {
   ...
   canMove = (moveDir.x < -0.5f || moveDir.x > +0.5f) && !Physics.CapsuleCast(transform.position, transform.position +
   ...
   else
   {
    ...
    canMove = (moveDir.z < -0.5f || moveDir.z > +0.5f) && !Physics.CapsuleCast(transform.position, transform.position +
